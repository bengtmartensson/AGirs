 ...
 * changes state. It gives more detailed results than the 50�s interrupts of IRrecv
...
/*
 * The original IRrecv which uses 50�s timer driven interrupts to sample input pin.
 */
void IRrecv::resume() {
...
#else
unsigned long IRrecvBase::GAP_TICKS;

void IRrecvBase::setEndingTimeout(unsigned long timeOut) {
    GAP_TICKS = timeOut / USECPERTICK;
}

unsigned long IRrecvBase::getEndingTimeout() const {
...
ISR(TIMER_INTR_NAME)
{
  ...
  case STATE_IDLE: // In the middle of a gap
    if (irdata == IR_MARK) {
      if (irparams.timer < IRrecvBase::GAP_TICKS) {
